...
def generate_rows(fact_schema, fact, fact_to_meta_field):
    """
    Turns a schema with nested relationships into a set of flat rows
    """
    relationship_fields = [
        field for field in fact_schema.declared_fields if field != fact_to_meta_field]
    # print(relationship_fields)
    relationship_fields_max_len = max(
        map(lambda fieldname: len(fact[fieldname]), relationship_fields))
    # print(relationship_fields_max_len)
    result = []
...
class FeitenList(Resource):
    ...
    _excluded_fact_post_fields = []

    # Fields that cannot be used for SQL INSERT
    _excluded_meta_create_fields = ["UUID", "ID"]
    ...
    def __init__(self, meta_schema, meta_tablename, fact_schema, fact_tablename, fact_to_meta_field):
        ...
        self.meta_tablename = meta_tablename
    ...
    def post(self):
        ...
        excluded_fields = self._excluded_fact_post_fields + self._excluded_meta_post_fields
        allowed_fields = [
        ...
        linked_rows = generate_rows(
            fact_schema, fact_object, self._fact_to_meta_field)
        db = records.Database(db_connection_string)
        ...
        for field in meta_schema.declared_fields:
            if field not in self._excluded_meta_create_fields:
                if meta_schema.declared_fields[field].attribute:
                    meta_create_fields += f"[{field.attribute}], "
                    meta_create_values += f":{field.attribute}, "
                else:
                    meta_create_fields += f"[{field}], "
                    meta_create_values += f":{field}, "
        meta_create_fields = meta_create_fields[:-2]
        meta_create_values = meta_create_values[:-2]

        meta_create_query = f'''INSERT INTO [dbo].[{self.meta_tablename}] ({meta_create_fields}) VALUES ({meta_create_values})'''
        result = db.query(meta_create_query, **{key: value for key, value in meta_object.items() if key not in self._excluded_meta_create_fields})
        return str(result)
