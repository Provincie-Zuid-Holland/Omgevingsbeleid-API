...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        FROM publication_versions AS v
        JOIN publications p ON v."Publication_UUID" = p."UUID"
        JOIN publication_environments e ON p."Environment_UUID" = e."UUID"
        WHERE e."Has_State" = false
    """)
    ...
    op.execute("""
        WITH announcement_statuses AS (
            ...
            FROM publication_announcements AS pa
        ...
        latest_packages AS (
            ...
            FROM (
                SELECT 
                    ...
                    pap."Package_Type",
                    p."Procedure_Type",
                    COALESCE(ans.has_pending_announcement, 0) as has_pending_announcement,
                    COALESCE(ans.has_valid_announcement, 0) as has_valid_announcement,
