...
FILES_DIR: Path = BASE_FILES_DIR / "publication_storage_files"


class PublicationStorageFileSpec(Spec):
...
class PublicationStorageFilePrefillHandler(BasePrefillHandler[PublicationStorageFileSpec]):
    def fill(
        self, record: Record[PublicationStorageFileSpec], context: PrefillContext
    ) -> Record[PublicationStorageFileSpec]:
        ...
        # Process the image
        path = Path(FILES_DIR / record.spec.File_Path)
...
class PublicationStorageFilePersistHandler(BasePersistHandler[PublicationStorageFileSpec]):
    def to_rows(self, record: Record[PublicationStorageFileSpec], context: PersistContext) -> Sequence[Base]:
        spec: PublicationStorageFileSpec = record.spec
        return [
