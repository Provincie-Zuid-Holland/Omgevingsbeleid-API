...
def test_can_change_module_manager(admin: TestClient, ctx: Context):
    ...
    response = admin.post("/modules/1", json={"module_manager_1_id": str(new_manager)})

    assert response.status_code == 200, response.text
    module = ctx.session.get(ModuleTable, 1)
    assert module
    assert module.module_manager_1_id == new_manager
...
def test_duplicate_managers_returns_400(admin: TestClient, ctx: Context):
    ...
    response = admin.post(
        "/modules/1",
        json={"module_manager_1_id": str(manager), "module_manager_2_id": str(manager)},
    )
