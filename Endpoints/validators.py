...
whitelist_tags = ['h1', 'h2', 'h3', 'h4', 'h5', 'p', 'b', 'i','strong','li', 'ol', 'ul', 'img', 'br', 'u', 'em', 'span', 'sub']
                    ...
                    for style in style_lines:
                        if style:
                            ...
                            if style_name not in whitelist_style:
                                raise ValidationError(f'Non whitelisted style attribute "{style_name}" in text "{el}"')           
                    continue
                elif attr not in whitelist_attrs:                    
                    raise ValidationError(f'Non whitelisted attribute "{attr}" in text "{el}"')
                uri_parts = urlparse(val)
                if all([uri_parts.scheme, uri_parts.netloc, uri_parts.path]):
                    if uri_parts.scheme not in whitelist_schema:
                        raise ValidationError(f'Non whitelisted url schema "{val}" in text "{el}"')
        else:
