...
class SearchEndpointBuilder(EndpointBuilder):
    ...
    def _compute_used_columns(
        ...
        objects_set: set[str] = {c.name for c in ObjectsTable.__table__.columns}
        module_set: set[str] = {c.name for c in ModuleObjectsTable.__table__.columns}
        ...
        for model_id in model_ids:
            model: Model | DynamicObjectModel = models_provider.get_model(model_id)
        ...
        final_columns.update(
            ...
            ["id", "modified_date", "object_type", "title", "description"]
        )
