...
def _reset_fulltext_index(
    ...
    if exists:
        session.execute(DDL(f"DROP FULLTEXT INDEX ON {table_name}"))
...
def mssql_setup_search_database(
    ...
    with session_scope_with_context(db_session_factory) as session:
        ...
        result = session.execute(
            text(f"SELECT name FROM sys.fulltext_stoplists WHERE name = '{mssql_search_stoplist_name}';")
        )
        exists = bool(result.all())
        if not exists:
            print("Create fulltext stoplist")
            session.execute(DDL(f"CREATE FULLTEXT STOPLIST {mssql_search_stoplist_name};"))
        ...
        if not exists:
            ...
            session.execute(DDL(f"CREATE FULLTEXT CATALOG {mssql_search_ftc_name};"))
