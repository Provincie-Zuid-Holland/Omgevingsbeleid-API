import uuid
from typing import Annotated, List, Optional, Dict, Any, Sequence, Tuple
...
from app.api.domains.modules.types import ModuleObjectActionFull, ModuleObjectContextShort, ModuleObjectShort, ModuleStatusCode, ObjectStaticShort
from app.api.domains.objects.types import ObjectStatics
from app.api.domains.users.dependencies import depends_current_user
from app.api.endpoint import BaseEndpointContext
from app.api.utils.pagination import OptionalSortedPagination, OrderConfig, PagedResponse, PaginatedQueryResult, Sort, SortedPagination
...
class ModuleObjectsResponse(BaseModel):
    Module_ID: int
    Module_Latest_Status: str

    Model: BaseModel # Parsed from ObejctTable via Object parser
...
def get_list_module_objects_endpoint(
    ...
    paginated_items: Sequence[Tuple[ModuleObjectsTable, ObjectStaticsTable, ModuleObjectContextTable, str]] = paginated_result.items

    rows: List[ModuleObjectsResponse] = []
    for (object_table, object_static, module_object_context, module_status) in paginated_items:
        parsed_model: BaseModel = module_objects_to_models_caster.cast(object_table, context.model_map)
        rows.append(
