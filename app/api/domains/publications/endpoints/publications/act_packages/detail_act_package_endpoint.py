...
class PublicationActPackageDetailItem(BaseModel):
    UUID: uuid.UUID
    Package_Type: str
    Report_Status: str
    Delivery_ID: str
...
def get_detail_act_package_endpoint(
    ...
    query = package_repository.build_detail_query(act_package_uuid)
    row = session.execute(query).first()

    if row is None:
        raise HTTPException(status_code=404, detail="Package not found")

    return PublicationActPackageDetailItem.model_validate(row)
