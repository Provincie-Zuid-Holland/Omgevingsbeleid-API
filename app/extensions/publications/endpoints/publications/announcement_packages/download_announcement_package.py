...
from app.extensions.publications.dependencies import depends_publication_zip_by_announcement_package
...
class DownloadPublicationAnnouncementPackageEndpoint(Endpoint):
    def __init__(self, path: str):
        self._path: str = path

    def register(self, router: APIRouter) -> APIRouter:
        def fastapi_handler(
            package_zip: PublicationPackageZipTable = Depends(depends_publication_zip_by_announcement_package),
            user: UsersTable = Depends(
                depends_current_active_user_with_permission_curried(
                    PublicationsPermissions.can_download_publication_announcement_package_report,
                ),
            ),
...
class DownloadPublicationAnnouncementPackageEndpointResolver(EndpointResolver):
    def get_id(self) -> str:
    ...
    def generate_endpoint(
        ...
        return DownloadPublicationAnnouncementPackageEndpoint(path=path)
