...
class Maatregelen_Schema(Dimensie_Schema):
    ...
    Status = MM.fields.Str(required=True, validate= [MM.validate.OneOf(["Ontwerp", "Vigerend"])], obprops=[])

    @MM.post_load
    def toelichting_to_raw(self, data, **kwargs):
        data['Toelichting_Raw'] = str(html.fromstring(data['Toelichting']).text_content())
        return data
